...
public class VietPayFtpJob
extends AbstractJob {
    private Log log = LogFactory.getLog((Class)VietPayFtpJob.class);
    ...
    public void execute() {
        ...
        try {
            lastImportDate = new Long(configDao.getValue("cdrsync", "cacher", "vietpay.last-import-date"));
        }
        catch (Exception e1) {
            lastImportDate = null;
        ...
        airtimeConfig.setParamKey("vietpay.last-import-date");
        if (maxDateFile == null) {
    ...
    public static void main(String[] args) {
        VietPayFtpJob cdrSyncJob = new VietPayFtpJob();
        cdrSyncJob.execute();
    }
    ...
    implements FTPFileFilter {
        ...
        public boolean accept(FTPFile ftpFile) {
            if (ftpFile.isFile() && ftpFile.getName().length() == 26 && ftpFile.getName().endsWith("_vietpay.txt") && ftpFile.getSize() > 0) {
                if (this.lastImportDate != null) {
                    try {
                        ...
                        VietPayFtpJob.this.log.error(("Invalid fileName: " + ftpFile.getName()));
                        return false;
                    }
                }
                return true;
