...
(function($) {
  $.number_format = function(value, opts) {
    ...
    function get_numeric(val) {
      var multiply = 1;
      var decimal = val.lastIndexOf(options.decimal);
      if (decimal!=-1) {
    	if (val.length-decimal-1>options.precision)
    		val = val.substring(0, decimal+options.precision+1);
    	else {
    		val = val + "000000000".substring(0, options.precision - (val.length-decimal-1));
    	}
      }
      else {
    	  val = val + ".000000000".substring(0, options.precision+1);
      }
    ...
    if(get_numeric(value) == 0 || get_numeric(value) == '')
      return signal + options.defaultValue;
