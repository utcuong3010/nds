...
public class VnPayCdrForward implements IForward {
	private static Log log=LogFactory.getLog(VnPayCdrForward.class);
	...
	private static class  VnPayCdrForwardHolder{
		private static VnPayCdrForward instance=new VnPayCdrForward();
	}

	private VnPayCdrForward() {
		errSeqMap=new ConcurrentHashMap<Long, CdrSync>();
	}
	
	public static VnPayCdrForward getInstance(){
	...
	public boolean execute(ContextBase context){
		...
		try{
			...
			ctxRequest.put(Attributes.ATTR_QUEUE_ID, AppConstants.VNPAY_SETTINGS.getCdrProcessorQueueId());
			ctxRequest.put(Attributes.ATTR_DEST_WFP, AppConstants.VNPAY_SETTINGS.getCdrProcessorWfp());
//				backup txn id
...
//							cdrSync.setpStatus(AppConstants.TXN_STATUS_SUCCESS);
						}
					}
//					dataDao.insert(cdrSync);
	...
	private void checkForAlert(ContextBase context) {
		if ( errSeqMap.size()>= AppConstants.VNPAY_SETTINGS.getMaxSeqError()){
	...
	private CdrSync getCrdData(ContextBase context){
		...
		cdrSync.setProviderId(AppConstants.VNPAY_SETTINGS.getConnectionType());
		...
		cdrSync.setpTxnId("");
		cdrSync.setpStatus("");
		if (AppConstants.VNPAY_SETTINGS.isSuccessRc(cdrSync.getErrorCode()))
			cdrSync.setStatus(AppConstants.TXN_STATUS_SUCCESS);
		else
			if (AppConstants.VNPAY_SETTINGS.isNoResponse(cdrSync.getErrorCode()))
