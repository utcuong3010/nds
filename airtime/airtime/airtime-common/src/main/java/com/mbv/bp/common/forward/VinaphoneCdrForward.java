...
public class VinaphoneCdrForward implements IForward {
	private static Log log=LogFactory.getLog(VinaphoneCdrForward.class);
	...
	private static class  VietPayCdrForwardHolder{
		private static VinaphoneCdrForward instance=new VinaphoneCdrForward();
	}

	private VinaphoneCdrForward() {
		errSeqMap=new ConcurrentHashMap<Long, CdrSync>();
	}
	
	public static VinaphoneCdrForward getInstance(){
	...
	public boolean execute(ContextBase context){
		...
		try{
			...
			ctxRequest.put(Attributes.ATTR_QUEUE_ID, AppConstants.VINAPHONE_SETTINGS.getCdrProcessorQueueId());
			ctxRequest.put(Attributes.ATTR_DEST_WFP, AppConstants.VINAPHONE_SETTINGS.getCdrProcessorWfp());
//				backup txn id
...
//					dataDao.insert(cdrSync);
	...
	private void checkForAlert(ContextBase context) {
		if ( errSeqMap.size()>= AppConstants.VINAPHONE_SETTINGS.getMaxSeqError()){
	...
	private CdrSync getCrdData(ContextBase context){
		...
		cdrSync.setProviderId(AppConstants.VINAPHONE_SETTINGS.getConnectionType());
		...
		if (AppConstants.VINAPHONE_SETTINGS.isSuccessResponse(cdrSync.getErrorCode()))
			cdrSync.setStatus(AppConstants.TXN_STATUS_SUCCESS);
		else if (AppConstants.VINAPHONE_SETTINGS.isUnknownResponse(cdrSync.getErrorCode()))
