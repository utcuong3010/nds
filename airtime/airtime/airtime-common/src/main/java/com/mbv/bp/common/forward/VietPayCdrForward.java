...
public class VietPayCdrForward implements IForward {
	private static Log log=LogFactory.getLog(VietPayCdrForward.class);
	...
	private static class  VietPayCdrForwardHolder{
		private static VietPayCdrForward instance=new VietPayCdrForward();
	}

	private VietPayCdrForward() {
		errSeqMap=new ConcurrentHashMap<Long, CdrSync>();
	}
	
	public static VietPayCdrForward getInstance(){
	...
	public boolean execute(ContextBase context){
		...
		try{
			...
			ctxRequest.put(Attributes.ATTR_QUEUE_ID, AppConstants.VIETPAY_SETTINGS.getCdrProcessorQueueId());
			ctxRequest.put(Attributes.ATTR_DEST_WFP, AppConstants.VIETPAY_SETTINGS.getCdrProcessorWfp());
//				backup txn id
	...
	private void checkForAlert(ContextBase context) {
		if ( errSeqMap.size()>= AppConstants.VIETPAY_SETTINGS.getMaxSeqError()){
	...
	private CdrSync getCrdData(ContextBase context){
		...
		if (AppConstants.VIETPAY_SETTINGS.isSuccessResponse(cdrSync.getErrorCode()))
			cdrSync.setStatus(AppConstants.TXN_STATUS_SUCCESS);
		else if (AppConstants.VIETPAY_SETTINGS.isUnknownResponse(cdrSync.getErrorCode()))
