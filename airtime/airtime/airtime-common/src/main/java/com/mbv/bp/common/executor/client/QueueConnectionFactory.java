...
public class QueueConnectionFactory implements IObjectFactory<IExecutable>{
	private static Log logger = LogFactory.getLog(QueueConnectionFactory.class);
//	String address;
//	int port;
	ExecutorCfg cfg;
	public QueueConnectionFactory(ExecutorCfg cfg) {
...
//		this.port=port;
	...
	public IExecutable createObject() {
		...
		if (!cfg.isLocalEnable()){
			executable=new QueueConnection(cfg.getHost(), cfg.getPort()); 
			try{
				executable.connect();
