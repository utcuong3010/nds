...
#echo $CURPATH
echo "Setting up airtime_anypay system in $SRCPATH"
DEPLOYDIR="/opt/airtime-anypay-$APPVER"
...
echo ============================================================
if [ -f "$SRCPATH/src/main/bin/airtime_anypay" ];
then
	...
	make
	cp jsvc $SRCPATH/src/main/bin/airtime_anypay
fi;
...
unzip $SRCPATH/target/airtime-anypay-$APPVER-bin.zip -d /opt
if [ ! "$?" = 0 ]; then
...
chmod a+x $DEPLOYDIR/bin/airtime_anypay.sh
