...
#echo $CURPATH
echo "Setting up airtime-util system in $SRCPATH"
DEPLOYDIR="/opt/airtime-util-$APPVER"
...
echo ============================================================
if [ -f "$SRCPATH/src/main/bin/airtime_util" ];
then
	...
	make
	cp jsvc $SRCPATH/src/main/bin/airtime_util
fi;
...
unzip $SRCPATH/target/airtime-util-$APPVER-bin.zip -d /opt
if [ ! "$?" = 0 ]; then
...
chmod a+x $DEPLOYDIR/bin/airtime_util.sh
